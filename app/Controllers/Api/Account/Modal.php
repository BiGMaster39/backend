...
class Modal extends PrivateController
{
    private $apps;
    private $navigation;
    private $common;
    ...
    function __construct()
    {
        ...
        $this->navigation = new ModalNavigationModel();
    ...
    /**
     * Get all modal navigation items
     * @param string $uid
     * @return mixed
     */
    ...
    public function update(int $id = 0)
    {
        ...
        $app = $this->apps
            ->where(["id" => $item["app_id"], "user" => $this->user["id"]])
            ->select("id,uid")
            ->first();
        if (!$app) {
            return $this->respond([
    ...
    /**
     * Get validation rules for create new modal item
     * @return array
     */
    private function create_validation_type(): array
    {
        return [
            "icon"         => ["label" => lang("Fields.field_22"),  "rules" => "required|min_length[3]|max_length[50]"],
